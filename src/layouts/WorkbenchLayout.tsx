    ...
    return (
        <div className="flex h-screen w-full overflow-hidden bg-background text-foreground transition-colors">
            <PanelGroup direction="horizontal">
                {/* Sidebar */}
                <Panel defaultSize={20} minSize={15} maxSize={30} className="flex shrink-0 z-20">
                    <WorkbenchSidebar />
                </Panel>

                {/* Resize Handle */}
                <PanelResizeHandle className="w-1.5 bg-border/40 hover:bg-primary/40 active:bg-primary/60 transition-colors shadow-inner flex items-center justify-center cursor-col-resize group z-30">
                    <div className="w-0.5 h-10 bg-muted-foreground/30 rounded-full group-hover:bg-primary/50 transition-colors" />
                </PanelResizeHandle>

                {/* Main Content Area */}
                <Panel className="flex flex-col min-w-0 bg-background relative z-10 shadow-[-10px_0_15px_-5px_rgba(0,0,0,0.05)]">
                    ...
                    <header className="h-16 border-b border-border/80 bg-background/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0 transition-all relative">
                        ...
                        <div className="flex items-center gap-3 ml-4">
                            ...
                            <div className="h-7 w-px bg-border/60 mx-2" />
