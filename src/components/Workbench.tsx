...
const Workbench = () => {
    ...
    useGSAP(() => {
        ...
        tl.to(".wb-script-text", { textShadow: "0 0 20px hsla(var(--signal), 0.8), 0 0 40px hsla(var(--signal), 0.4)", color: "hsl(var(--foreground))", duration: 1 });
    ...
    return (
        <section ref={containerRef} className="relative h-screen bg-background flex flex-col items-center justify-center overflow-hidden">
            ...
            {/* Background Grid */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(var(--signal),0.03)_0%,transparent_70%)]" />
            ...
            <div ref={trackRef} className="relative z-10 w-full max-w-4xl px-6 text-center">
                ...
                <div className="wb-step-1 absolute inset-0 flex flex-col items-center justify-center">
                    <div className="w-24 h-24 rounded-full bg-card border border-border flex items-center justify-center mb-6 shadow-2xl shadow-signal/20">
                        <AudioLines size={40} className="text-signal" />
                    </div>
                    ...
                    <div className="mt-12 flex items-center gap-1 h-16">
                        {Array.from({ length: 20 }).map((_, i) => (
                            <div key={i} className="w-2 bg-signal/50 rounded-full" style={{ height: `${Math.random() * 100}%` }} />
                        ))}
                    </div>
                </div>
            ...
            <div className="absolute bottom-10 left-0 right-0 flex justify-center gap-2">
                <div className="w-2 h-2 rounded-full bg-signal/50" />
